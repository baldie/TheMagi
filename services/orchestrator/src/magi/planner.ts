...
`    {
      "Step4": {
        "instruction": "respond directly to the user's message with a concise answer"
      }
    }`;
...
USAGE:\n${t.instructions || 'Infer instructions based on parameter names'}`;
  ...
  static getSeedPlan(): PlanStep[] {
    return [
      { instruction: 'Create response plan for User\'s Message', type: StepType.PLAN_CREATION },
      { instruction: 'Format the plan for parsing and execution', type: StepType.PLAN_FORMAT },
      { instruction: 'Execute custom plan', type: StepType.PLAN_EXPANSION }
    ];
  }

  /**
   * Generate a step-by-step plan for addressing a User's Message.
   * @param userMessage - The User's Message to analyze.
   * @returns A promise that resolves to the raw step-by-step plan text.
   */
  async createResponsePlan(userMessage: string): Promise<string> {
    let prompt = `
INSTRUCTIONS:
    Your task is to understand the User's Message and create a step-by-step plan of how you will address it.
    Keep your plan focused and efficient - aim for 3-8 steps maximum.
    Never exceed ${MAX_PLAN_STEPS} steps and always start on Step4 (Step1, Step2, and Step3 are reserved).
    ${PERSONAS_CONFIG[this.magiName].uniqueInstructions || ''}
    
    CRITICAL: The execution of this plan is a sequential process. The output or results generated by each step will be directly consumed as input by the immediate next step. Therefore, if a later step requires specific information (like retrieved user data) that is produced by an earlier step, you MUST use the <PLACEHOLDER> token. This signifies that the exact content from the preceding step's output will be dynamically inserted at that point during execution.

    If any of the steps require using a tool, you must specify the tool name and arguments in that step.
    Each step is atomic, so the instructions should match what is being done with the tool in that step.
    ...
    Here are the tool(s) you have access to:
    ${this.toolsList}
        
    Below are examples of different types of messages and appropriate planning approaches:

    USER MESSAGE:
    "I will wash my hands."
    
    REASONING:
    Since this is a statement with no question the plan can have a single step with an instruction to respond directly with acknowledgement.

    PLAN:
    Step4: Respond directly with acknowledgement.

    USER MESSAGE:
    "What should I make for dinner that is simple and healthy?"`;
    ...
    switch(this.magiName){
      case MagiName.Balthazar:
        prompt += `
        REASONING:
        Since this is somewhat of a complex inquiry, I will create a multi step plan that starts with a web search and closes in on an answer.

        PLAN: 
        Step4: Search the web for dinner options from reputable sources using tavily-search TOOL with query "healthy simple dinner options", max_results 3, topic "general", include_answer true
        Step5: Evaluate the search results from the previous step (<PLACEHOLDER>). Select the most promising URL(s) for a healthy and simple recipe. Provide these URL(s) as the direct output of this step, which will be consumed by the next step.
        Step6: Use the tavily-extract TOOL to get detailed content from the URL(s) provided in the previous step's output, using <PLACEHOLDER> for the urls parameter
        Step7: Summarize the meal suggestions and recipes from the extracted content of the previous step. Include clear citations (where you found them).`;
        break;

      case MagiName.Melchior:
        prompt += `
        REASONING:
        In order to provide a tailored response, it might be worth checking if there is any pertinent data on file related to the inquiry.

        PLAN:
        Step4: Gather any food-related information on file for the user using personal-data TOOL with action "retrieve", categories ["food preferences", "dietary restrictions", "food allergies"], user_context "gathering meal planning information"
        Step5: Based on the retrieved user data from the previous step (<PLACEHOLDER>), propose a list of popular, simple, and healthy meal options. Justify the suggestions based on the user's preferences, restrictions, and allergies.

        USER MESSAGE:
        "I love spicy food"

        REASONING:
        While this is a simple statement, it provides some user preference information that might be useful later so I should store it.

        PLAN:
        Step4: Store the user's food preference for spicy food using personal-data TOOL with action "store", content "User loves spicy food.", category "food preferences", user_context "updating user food profile"
        Step5: Acknowledge that the preference has been stored and offer a general response.

        USER MESSAGE:
        "I drive a Tesla."
        
        REASONING:
        The user is providing me information about their vehicle, so I should store it.

        PLAN:
        Step4: Store the user's vehicle information using personal-data TOOL with action "store", content "User drives a Tesla.", category "vehicle information", user_context "storing user's vehicle information"
        Step5: Acknowledge the information has been stored and offer a general response.`;
        break;

      case MagiName.Caspar:
        prompt += `
        REASONING:
        There may be information available from the user's smart home devices relevant to this query so I'll create a plan to gather this data and synthesize it.

        PLAN:
        Step4: Access smart home devices to gather current kitchen and food information using smart-home-devices TOOL with device_types ["smart_fridge", "kitchen_lights", "kitchen_camera"], query_purpose "meal planning assistance"
        Step5: Process the raw data received from the smart home devices in the previous step. Identify and extract specific details: if a smart fridge is available, list its ingredients, expiration dates, and quantities; if a kitchen camera is available, assess if the kitchen is clear and identify any visible appliances. The output of this step will be a structured collection of all identified ingredients with their details, available appliances, and the current kitchen condition.
        Step6: Summarize the available ingredients, appliances, and the kitchen's condition, derived from the structured collection in the previous step.`;
        break;
    }
    ...
    RESONING:
    This is a simple question that doesn't require tools or complex research, the plan will simply be to respond to the inquiry directly with a concise answer.

    PLAN:
    Step4: Respond directly with a concise answer.
    
    Now ${this.magiName}, analyze the following user message and create your step-by-step plan. 
    Focus on WHAT you need to do and HOW you'll approach it. If you need to use a tool that you have access to, be sure to specify which tool and what parameters.
    Respond only with your step-by-step plan, do not include any other text.

    USER MESSAGE:
    "${userMessage}"`;
    
    logger.debug(prompt);
    const planResponse = await this.conduitClient.contact(prompt, '', this.model, { temperature: this.temperature });
    
    return planResponse;
  }

  /**
   * Format a raw step-by-step plan into the required JSON structure for parsing.
   * @param rawPlan - The raw step-by-step plan text from createResponsePlan.
   * @returns A promise that resolves to the JSON formatted plan.
   */
  async formatResponsePlan(rawPlan: string): Promise<string> {
    let prompt = `
INSTRUCTIONS:
    Your task is to take a step-by-step plan and format it into the required JSON structure.
    The plan should start with Step4 (Step1, Step2, and Step3 are reserved). Never exceed ${MAX_PLAN_STEPS} steps.
    Each step should have an "instruction" field and optionally a "tool" field with "name" and "args".
    Use the <PLACEHOLDER> token when a later step needs content from a previous step.
    ...
    Here are examples of the expected JSON format:

    PLAN:
    Step4: Respond directly with acknowledgement.

    REASONING:
    As this is a single step plan with no tool use, I will just make the instruction the Step4 text in my JSON.

    \`\`\`json
    {
      "Step4": {
        "instruction": "Respond directly with acknowledgement of the user's message."
      }
    }
    \`\`\``;
    ...
    switch(this.magiName){
      case MagiName.Balthazar:
        ...
        REASONING:
        Step4 and Step6 both mention TOOL use, so I wil include the JSON structure for the tool based on the step text. The argument for the tavily-extract tool in Step6 requires the URL from Step5 so I will use the <PLACEHOLDER> token for the urls parameter.
        ...
        \`\`\`json
        {
          ...
          "Step5": {
            "instruction": "Evaluate the search results from the previous step <PLACEHOLDER>. Select the most promising URL(s) for a healthy and simple recipe. Provide these URL(s) as the direct output of this step, which will be consumed by the next step."
          },
          "Step6": {
          ...
          "Step7": {
            "instruction": "Summarize the meal suggestions and recipes from the extracted content of the previous step. Include clear citations (where you found them)."
          }
        }
        \`\`\``;
        break;
      ...
      case MagiName.Melchior:
        ...
        PLAN:
        Step4: Gather any food-related information on file for the user using personal-data TOOL with action "retrieve", categories ["food preferences", "dietary restrictions", "food allergies"], user_context "gathering meal planning information"
        Step5: Based on the retrieved user data from the previous step (<PLACEHOLDER>), propose a list of popular, simple, and healthy meal options. Justify the suggestions based on the user's preferences, restrictions, and allergies. Share findings.

        REASONING:
        Step4 mentions TOOL use, so I wil include the JSON structure for the tool based on the step text.
        ...
        \`\`\`json
        {
          ...
          "Step4": {
            "instruction": "Based on the retrieved user data from the previous step (<PLACEHOLDER>), propose a list of popular, simple, and healthy meal options. Justify the suggestions based on the user's preferences, restrictions, and allergies. Share findings."
          }
        }
        \`\`\`
        ...
        PLAN:
        Step4: Store the user's food preference for spicy food using personal-data TOOL with action "store", content "User loves spicy food.", category "food preferences", user_context "updating user food profile"
        Step5: Acknowledge that the preference and allergy have been stored and offer a general response.

        REASONING:
        Step4 mentions TOOL use, so I wil include the JSON structure for the tool base on the step text in Step4's JSON.

        \`\`\`json
        {
          "Step4": {
            "instruction": "Store the user's food preference for spicy food.",
            "tool": {
              "name": "personal-data",
              "args": {
                "action": "store",
                "content": "User loves spicy food.",
                "category": "food preferences",
                "user_context": "updating user food profile"
              }
            }
          },
          "Step5": {
            "instruction": "Acknowledge that the preference and allergy have been stored, and provide relevant advice about spicy food options that are nut-free."
          }
        }
        ...
        {
          "Step4": {
            "instruction": "Store the user's vehicle information.",
            "tool": {
              "name": "personal-data",
              "args": {
                "action": "store",
                ...
                "user_context": "storing user's vehicle information"
              }
            }
          },
          "Step5": {
            "instruction": "Acknowledge the information has been stored, and offer a general response."
          }
        }
        \`\`\``;
        break;
      ...
      case MagiName.Caspar:
        ...
        {
          "Step4": {
            "instruction": "Access smart home devices to gather current kitchen and food information from smart home devices like a smart fridge, kitchen lights, and kitchen camera.",
            "tool": {
          ...
          "Step6": {
            "instruction": "Summarize the available ingredients, appliances, and the kitchen's condition, derived from the structured collection in the previous step."
          }
        }
        \`\`\``;
        break;
    ...
    prompt += `
    PLAN
    Step4: Respond directly to the user's message with a concise answer
    ...
    Now take the following step-by-step plan and format it into the required JSON structure.
    Only respond with the properly formatted JSON plan, do not include any other text.

    **CRITICAL:** For each step in the plan: If the step mentions using a TOOL, you MUST specify the tool name and arguments as provided in that step.
  ...
  async parsePlanSteps(planData: RawPlanData): Promise<PlanStep[]> {
    try {
      ...
      return [
        { instruction: 'Respond to the User\'s Message directly', type: StepType.PLAN_EXECUTION }
      ];
    }
  }
   ...
   * @param originalUserMessage - The original User's Message.
   * @returns A promise that resolves to the cumulative output.
   */
  async executePlan(steps: PlanStep[], originalUserMessage: string): Promise<string> {
      ...
      try {
        ...
        switch (step.type) {
          case StepType.PLAN_CREATION:
            // The Magi is creating the plan based on the User's Message
            stepResult = await this.createResponsePlan(originalUserMessage);
            break;

          case StepType.PLAN_FORMAT:
            // Here we take the high level, plain english plan, and create a JSON representation of it.
            stepResult = await this.formatResponsePlan(previousStepOutput);
            break;
          
          case StepType.PLAN_EXPANSION: {
            // Here we append the Magi's generated plan to the currently executing plan
            const rawPlanResponse = JSON.parse(previousStepOutput) as RawPlanData;
          ...
          default: {
            ...
            stepResult = await this.executeStep(step, stepNumber, previousStepOutput, originalUserMessage, steps);
            break;
          }
        }
   ...
   * @param originalUserMessage - The original User's Message for context
...
Your task:
- Consider the previous step output and the original User's Message.
  ...
  private async executeStep(
    ...
    if (step.toolName && step.toolParameters) {
      ...
      if (previousOutput?.trim()) {
        updatedStep = await this.hydrateToolParameters(step, previousOutput, originalUserMessage);
      } else {
      ...
      if (updatedStep.toolName == 'tavily-extract'){
        const cleanExtractPrompt = getCleanExtractPrompt(originalUserMessage, toolResponse);
      ...
      // Respond to this action with the instructions
      if (updatedStep.toolName == "personal-data") {
        logger.debug(`\n${toolResponse}\n`);
        const summarize = `You have just completed the following task:\n${updatedStep.instruction}\nThis resulted in:\n${toolResponse}\n\nNow, concisely summarize the action and result in plain language.`;
        toolResponse = await this.conduitClient.contact(summarize, '', this.model, { temperature: this.temperature })
      }
      ...
      const stepExecutionPrompt = `${this.magiName}, your original plan for addressing the User's Message is as follows:
      ${planContext}

      You are now executing executing step ${stepNumber} of your plan, therefore your current instructions are:\n"${step.instruction}"
      This is ultimately to address the User's Message: "${originalUserMessage}"
      ...
      If this is the last step, the results will be shared.`;
