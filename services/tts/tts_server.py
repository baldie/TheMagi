...
def initialize_model():
    ...
    if not CHATTERBOX_AVAILABLE:
        ...
        initialization_error = "Chatterbox TTS package not available - please install with 'pip install git+https://github.com/resemble-ai/chatterbox.git'"
    ...
    try:
        ...
        initialization_stage = "failed"
        initialization_error = f"Import error: {e} - Chatterbox TTS may not be properly installed"
        ...
        initialization_stage = "failed"
        if "CUDA" in str(e):
            initialization_error = f"CUDA error: {e} - Try installing CPU version or update GPU drivers"
...
async def startup_event():
    ...
    if success:
        ...
        logger.warning("Service will respond to health checks but TTS requests will fail")
        logger.warning("Check the logs above for specific error details")

@app.get("/health", response_model=HealthResponse)
async def health_check():
    """Health check endpoint with detailed status information"""
    global initialization_stage, initialization_error
    ...
    if tts_model is not None:
        ...
        status = "failed"
    elif initialization_stage in ["starting", "checking_dependencies", "configuring_device", "downloading_model"]:
        status = "initializing"
    else:
        status = "unhealthy"
...
async def synthesize_speech(request: TTSRequest):
    ...
    logger.debug(f"[{request_id}] Request details: text='{request.text[:100]}...', voice={request.voice}, exaggeration={request.exaggeration}, cfg_weight={request.cfg_weight}")
    
    try:
        if tts_model is None:
            logger.error(f"[{request_id}] TTS model not loaded - service not ready")
            raise HTTPException(status_code=503, detail="TTS model not loaded. Check service logs for initialization errors.")
        ...
        if request.audio_prompt_path and os.path.exists(request.audio_prompt_path):
            ...
            logger.info(f"[{request_id}] Using voice cloning with: {request.audio_prompt_path}")
        
        # Generate speech using Chatterbox TTS
        logger.info(f"[{request_id}] Calling tts_model.generate() with parameters: {generation_kwargs}")
        start_time = datetime.now()
        
        try:
            wav = tts_model.generate(request.text, **generation_kwargs)
            generation_time = (datetime.now() - start_time).total_seconds()
            logger.info(f"[{request_id}] Generation completed in {generation_time:.2f} seconds")
            ...
            logger.error(f"[{request_id}] Generation traceback: {traceback.format_exc()}")
            raise
        ...
        try:
            ...
            logger.info(f"[{request_id}] Audio saved successfully, file size: {file_size} bytes")
        except Exception as save_error:
            logger.error(f"[{request_id}] Failed to save audio: {save_error}")
            raise
        
        if not os.path.exists(audio_path):
            logger.error(f"[{request_id}] Audio file was not created at expected path: {audio_path}")
            raise HTTPException(status_code=500, detail="Failed to generate audio file")
        
        duration = wav.shape[-1] / tts_model.sr
        logger.info(f"[{request_id}] Synthesis completed successfully - Duration: {duration:.2f}s, Sample rate: {tts_model.sr}Hz")
        ...
        logger.error(f"[{request_id}] TTS synthesis failed with unexpected error: {str(e)}")
...
async def list_voices():
    ...
    return {
        ...
        "note": "Voice characteristics are achieved through exaggeration and cfg_weight parameters. Add audio_prompt_path for voice cloning."
    }
