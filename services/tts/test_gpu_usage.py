...
class GPUMonitor:
    ...
    def get_gpu_stats(self):
        ...
        try:
            result = subprocess.run([
                'nvidia-smi', 
                '--query-gpu=timestamp,name,memory.used,memory.total,utilization.gpu,utilization.memory,temperature.gpu,power.draw',
                    ...
                    if line.strip():
                        ...
                        if len(parts) >= 8:
                            stats.append({
                                ...
                                'gpu_util': int(parts[4]) if parts[4] != '[N/A]' else 0,
                                'memory_util': int(parts[5]) if parts[5] != '[N/A]' else 0,
    ...
    def analyze_results(self):
        ...
        for gpu_id, data in gpu_groups.items():
            ...
            power_draws = [entry['power_draw'] for entry in data if entry['power_draw'] > 0]
            ...
            if max_util > 50:
                print(f"✅ GPU was heavily utilized (max {max_util}%) - CUDA acceleration confirmed!")
            elif max_util > 10:
                print(f"✅ GPU was moderately utilized (max {max_util}%) - CUDA likely working")
            elif max_util > 0:
                print(f"⚠️  GPU had minimal utilization (max {max_util}%) - may not be using CUDA")
            else:
                print(f"❌ GPU showed no utilization - CUDA may not be working")
...
def test_tts_with_monitoring():
    ...
    try:
        ...
        # Wait a moment more to capture post-synthesis GPU usage
        print("\n📉 Collecting post-synthesis GPU usage...")
        time.sleep(2)
