...
def test_health_endpoint():
    ...
    try:
        ...
        print(
            ...
            f"{health_data.get('chatterbox_available', False)}"
        )
        print(
            "✓ Initialization Stage: "
            f"{health_data.get('initialization_stage', 'unknown')}"
        )
...
def test_status_endpoint():
    ...
    try:
        ...
        print(
            f"✓ Chatterbox Available: {status_data.get('chatterbox_available', False)}"
        )
    ...
    try:
        ...
        for voice_id, voice_info in voices_data.get("voices", {}).items():
            ...
            if settings:
                print(
                    f"    Settings: "
                    f"exaggeration={settings.get('exaggeration', 'N/A')}, "
                    f"cfg_weight={settings.get('cfg_weight', 'N/A')}"
                )
...
def test_speech_synthesis():
    ...
    try:
        ...
        print(
            "✓ Generation Time: "
            f"{synthesis_data.get('generation_time', 'unknown')} seconds"
        )
...
def main():
    ...
    if not cuda_available:
        print("✓ CUDA not available, no further GPU tests needed")
        return

    print_step(4, "Synthesizing Speech and Verifying GPU Usage")
    audio_id = test_speech_synthesis()

    if audio_id:
        print("✓ Speech synthesis test passed")
        test_audio_retrieval(audio_id)
    else:
    ...
    if tests_passed == total_tests:
        ...
        print(
            "⚠ Most tests passed, but there may be some issues. Check the output above."
        )
        return 1
    else:
